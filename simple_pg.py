# The expected return for a trajectory, tau:
# J(pi_theta) = E(R(tau)). We want to optimize this and need to
# compute grad_theta(J). This gradient can be expressed:
# grad = E(R(tau)* 
#          grad_theta(
#            Sum_i(  
#              log(policy(a_i|s_i))
...
#          )
#        )
# , where tau is sampled from 
# the current policy. The policy is a neural net that
# takes a state and returns the log-probabilities, i.e. 
# final activation function is nothing, and the logits are 
# interpreted as the log-probabilities of taking an action.
...
# loop over batch:
#   rewards = []
#   loop over number of episodes per batch
#      - sample a trajectory from current policy
#      - append reward to reward-list
#   compute J
#   run optimization for minimize negative J based on policy parameters
#   update policy parameters, implicitly done if using the tensorflow optimizaiton
...
def play_batch(batchsize, env, mod):
    ...
    for i_ep in range(batchsize):
        ...
        l_reward += [total_reward]*ts_in_ep
